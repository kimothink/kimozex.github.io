...
#include "Externvar.h"
#include <process.h>    // ham 2005.11.29
...
unsigned __stdcall ThreadProc(LPVOID lpParameter)  // ham 2005.11.29
{
	...
	sStartTotUserCard l_sStartTotUserCard;
    sInitUserCard l_sInitUserCard;      // ham 2006.02.27
    sUserChange l_sUserChange;          // ham 2006.02.27
			...
			if ( bErrorCode != ERROR_NOT ) {
				...
				switch(bCmd)
				{
					...
					case NGSC_STARTGAME:
						...
						g_biJackPotMoney = l_sStartTotUserCard.biJackPotMoney;
						g_bChoiceRoom = l_sStartTotUserCard.bChoiceRoom;
						...
						nRoomMasterArrayNo = g_CGameUserList.FindUser( l_sStartTotUserCard.nRoomMasterUniqNo );

						//게임이끝이나면 초기화를 하기때문에..임명해준다.
						//새로운 선을 임명.
						g_CGameUserList.NewSunSet( l_sStartTotUserCard.nRoomMasterUniqNo );

						//새로운 방장을 임명.
						g_CGameUserList.NewBangJangSet( l_sStartTotUserCard.nBbanJjangUniqNo );
						...
						//선 다음부터 차례대로 카드가 왔기 때문에. 찾아서 넣어주면 된다.
						for ( i = 0; i < nInitCardCnt; i++ ) 
						{

							for ( j = 0; j < MAX_ROOM_IN; j++ )
							{
								if ( l_sStartTotUserCard.l_sStartPerUserCard[j].nUserUniqNo <= 0 ) 
									continue;

                                //패킷에 담아져 있는 순서대로 넣어주면 된다.
                                nSlotNo = g_CGameUserList.FindUser( l_sStartTotUserCard.l_sStartPerUserCard[j].nUserUniqNo );

#ifdef SHOWUSERCARD
								g_pCMainFrame->UserCardAdd( l_sStartTotUserCard.l_sStartPerUserCard[j].byCardNo[i], nSlotNo );
#else								
								if ( nSlotNo != 0 ) 
								{
						...
						g_pCMainFrame->SetJackPotMoney( l_sStartTotUserCard.biJackPotMoney );
						g_pCMainFrame->m_bStartInit = TRUE;
						g_pCMainFrame->SetPanMoney( 0, l_sStartTotUserCard.biRoomCurMoney ); 
                        ...
                        /*
					case NGSC_STARTGAME:
						memcpy( &l_sStartUserCard, &szRealBufpos[nIndex], sizeof(l_sStartUserCard) );
						nIndex += sizeof(l_sStartUserCard);

						g_biJackPotMoney = l_sStartUserCard.biJackPotMoney;
						g_bChoiceRoom = l_sStartUserCard.bChoiceRoom;
						...
						nRoomMasterArrayNo = g_CGameUserList.FindUser( l_sStartUserCard.nRoomMasterUniqNo );

						//게임이끝이나면 초기화를 하기때문에..임명해준다.
						//새로운 선을 임명.
						g_CGameUserList.NewSunSet( l_sStartUserCard.nRoomMasterUniqNo );

						//새로운 방장을 임명.
						g_CGameUserList.NewBangJangSet( l_sStartUserCard.nBbanJjangUniqNo );
...
#ifdef SHOWUSERCARD
                                g_pCMainFrame->UserCardAdd( l_sStartUserCard.byCardNo[i], nUserArrayNo );
#else
								//자기 자신이 아니면.
                                if ( nUserArrayNo != 0 ) {
						...
						g_pCMainFrame->SetJackPotMoney( l_sStartUserCard.biJackPotMoney );
						g_pCMainFrame->m_bStartInit = TRUE;
						g_pCMainFrame->SetPanMoney( 0, l_sStartUserCard.biRoomCurMoney ); 
                    ...
                    // 버릴카드를 선택하면 초기카드 3장을 보내준다.
                    case NGSC_INITUSERCARD:
                        memset( &l_sInitUserCard, 0x00, sizeof( l_sInitUserCard ));
                        memcpy( &l_sInitUserCard, &szRealBufpos[nIndex], sizeof( l_sInitUserCard ));
                        nIndex += sizeof( l_sInitUserCard );

#ifdef SHOWUSERCARD
                        nSlotNo = g_CGameUserList.FindUser( l_sInitUserCard.nUserUniqNo );

                        for ( i = 0; i < 3; i++ ) {
                            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].nCardNo = l_sInitUserCard.nUserCard[i];
                        }
                    ...
                    case NGSC_USERCHANGE:
                        ...
                        nSlotNo = g_CGameUserList.FindUser( l_sUserChange.nUserUnqiNo );

                        g_pCMainFrame->ChangeUserCard( nSlotNo, &l_sUserChange );
#endif
                        break;
					...
					case NGSC_SETGAMECURUSER:
						...
						g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserInfo.biUserMoney = l_sSetGameCurUser.biBeforeGameUserMoney;

						g_pCMainFrame->UserSelectView( nSlotNo, l_sSetGameCurUser.cBeforeUserSelect, TRUE );
						
#ifndef SHOWUSERCARD
						//이전 유저가 Die를 하였으면 Die한 유저의 카드를 덮는다.
						if ( l_sSetGameCurUser.cBeforeUserSelect == '7' ) {
							for ( i = 0; i < g_CGameUserList.m_GameUserInfo[nSlotNo].m_nCardCnt; i++ ) {
								g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].nCardNo = 255;
							}
						}
						...
						g_pCMainFrame->SetPanMoney( nSlotNo, l_sSetGameCurUser.biRoomCurMoney);
						
						nSlotNo = g_CGameUserList.FindUser( l_sSetGameCurUser.nCurGameUserUniNo );
						g_pCMainFrame->SetUserCall( l_sSetGameCurUser.l_sGameCurUser, nSlotNo );
...
//                       
//                      BigInt M1=l_sEndGame.biCallGameUserMoney;
//						BigInt M2=l_sEndGame.biPureWinnerMoney;
//						int    M3=l_sEndGame.nCallUserUniNo;
//						BigInt M4=l_sEndGame.biCallGameUserMoney;
                        
						TRACE(" %d-%d-%d-%d\n",l_sEndGame.biCallGameUserMoney,l_sEndGame.biPureWinnerMoney,l_sEndGame.nCallUserUniNo,l_sEndGame.biCallGameUserMoney);
						
                     //   int M2=l_sEndGamen.
                      //  TRACE("l_sEndGame.biCallGameUserMoney %d\n",M);
						...
						if ( l_sEndGame.nJackPot == 1 || l_sEndGame.nJackPot == 2 || l_sEndGame.nJackPot == 3  ) {

							g_biUserJackPotMoney = l_sEndGame.biUserJackPotMoney;


							//판머니에 잭팟을 더해준다.(결과창 표시에서 +딴머니로 찍힌다)
							g_biPureWinMoney += g_biUserJackPotMoney;
							g_pCMainFrame->m_bJackPot = TRUE;
							g_pCMainFrame->PlaySound(g_Sound[GS_JACKPOT]);
						}
					...
					case NGSC_MYUSERJOKBO:
						...
						if ( g_CGameUserList.m_GameUserInfo[0].m_nCardCnt <= 6 )
							memcpy( g_CGameUserList.m_GameUserInfo[0].m_szJokboCard, &l_sMyUserJokBo, sizeof(l_sMyUserJokBo) );
						else
							memcpy( g_CGameUserList.m_GameUserInfo[0].m_szTempJokboCard, &l_sMyUserJokBo, sizeof(l_sMyUserJokBo) );

...
//레코드셋 열고..
vsMDate->vt= VT_DATE;
vsMDate->date= m_pRecordset->Fields->GetItem(long(6))->Value.date;

 DBVARIANT에서 VT_DATE타입, 
date로 오거든요, 그래서 이것을  MFC 클래스인 COleDateTime에다 넣어서 쓰고 있지요

varchar는CString으로 받고
number는 DECIMAL로 받으시면 됩니다...

DECIMAL에 대해서는 MSDN에서 찾아보시길... ㅡ.ㅡ
...
public:
	...
	BOOL GetFieldValue(LPCTSTR lpFieldName, __int64& inValue );
...
extern CADODatabase m_pAdoDB;
extern CADORecordset m_pAdoRS;

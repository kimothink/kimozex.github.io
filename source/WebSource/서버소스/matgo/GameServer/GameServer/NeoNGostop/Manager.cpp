...
void CManager::Run()
{
	...
	//yun 처음에 m_ServerShutdown 을 false 로 하고 m_ShutdownRemain 은 0이다.
...
{
	if( m_nKeepAliveSendTime >= PULSE_CONNECT_TIMEOUT)   {
...
int CManager::NewDescriptor(int s)
{
	...
	*newd->m_Output = '\0';
	...
	sprintf( a_Log, "(NewDescriptor) - newd->m_MyInfo.l_sUserInfo.nUserNo : %d", newd->m_MyInfo.l_sUserInfo.nUserNo );
	LogFile (a_Log);	

	if (++last_desc == 100000000)
		last_desc = INIT_USER_NO;

	sprintf( a_Log, "(NewDescriptor) - last_desc : %d", last_desc );
	LogFile (a_Log);	
...
bool CManager::MakeCSConThread()
{
	if ((m_hCSConThread = _beginthread( CSServerConThread, sizeof(CDbQ *), m_CSServerCon )) == -1)
		return false;

	m_CSServerCon->SetThread(m_hCSConThread);
	Log ("User-CSServerConThread Start");
	return true;
}


bool CManager::MakeCounterThread()
{
	if ((m_hCounterThread = _beginthread( CounterThreadProc, sizeof(CDbQ *), NULL)) == -1)
		return false;

	Log ("User-CounterThread Start");
	return true;
}
	...
	switch (STATE(d)) {
		...
		case GETUSER:
			...
			if ( ( 0 <= d->m_nRoomNo && d->m_nRoomNo < MAX_ROOMCNT ) &&
				...
				if ( g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].m_RoomList[nRoomNo].l_sRoomList.l_sRoomInfo.cGameDivi == '0' ) {
					...
					#ifdef PACKETANALY
						if ( d != NULL ) {
							sprintf( a_Log, "(CloseSocket)UserId : %s 게임이 대기중이라서 삭제한다.", d->m_MyInfo.szUser_Id );
							LogFile (a_Log);
						}
					#endif
				}
					...
					#ifdef PACKETANALY
						if ( d != NULL ) {
							sprintf( a_Log, "(CloseSocket)UserId : %s 게임중이라서 소켓리스트에서만 삭제한다.", d->m_MyInfo.szUser_Id );
							LogFile (a_Log);
						}
					#endif
					...
					for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
						...
						if ( g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].m_RoomList[nRoomNo].m_User[i] == NULL ) {
							...
							#ifdef PACKETANALY
								sprintf( a_Log, "(CloseSocket)팅긴놈이 NULL이네." );
								LogFile (a_Log);						
							#endif

							continue;
						}
						if ( g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].m_RoomList[nRoomNo].m_User[i]->m_bClosed == true )
							nCnt++;

							#ifdef PACKETANALY
								if ( d != NULL ) {
									sprintf( a_Log, "(CloseSocket)팅긴수 nCnt = %d", nCnt );
									LogFile (a_Log);
								}
							#endif
					...
					if ( nCnt >= MAX_ROOM_IN ) {  
						...
						for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
							...
							#ifdef PACKETANALY
								if ( d != NULL ) {
									sprintf( a_Log, "(CloseSocket)모두 팅겨서 방을 없앤다. nRoomNo = %d", nRoomNo );
									LogFile (a_Log);
								}
							#endif

							m_pDbQ->EraseQ (g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].m_RoomList[nRoomNo].m_User[i]);	// erase Descriptor from queue.
							STATEDBMODE(g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].m_RoomList[nRoomNo].m_User[i]) = GAMING_EXITUPDATE;
							g_pDbQ->AddToQ(g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].m_RoomList[nRoomNo].m_User[i]);	

						}
						
						g_World.m_Channel[d->m_MyInfo.l_sUserInfo.nChannel].DeleteRoom(nRoomNo);

					#ifdef REALTIME_REFRESH	
						memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
						nSendPacketLen = NGFrameWaitInfoChange( g_ClientSendBuf, d, d->m_nRoomNo, d->m_nRoomInUserNo, d->m_MyInfo.l_sUserInfo.nUserNo, 'X' );
						BroadCastUserPage( g_ClientSendBuf, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nUserPage, nSendPacketLen );			//대기실 해당페이지만.
					#endif	
						...
						memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
						nSendPacketLen = NGFrameGameingExiUser( g_ClientSendBuf, d );
						BroadCastRoomInUser( g_ClientSendBuf, d, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d->m_nRoomInUserNo, nSendPacketLen, true );


						#ifdef PACKETANALY
							if ( d != NULL ) {
								sprintf( a_Log, "(CloseSocket)한놈이 팅김을 알려준다. nRoomNo = %d, m_nRoomInUserNo = %d", d->m_nRoomNo, d->m_nRoomInUserNo );
								LogFile (a_Log);
							}
						#endif

...
void QuickJoin( CUserDescriptor *d, char *pReceBuffer ) 
{
...
void GameStart( CUserDescriptor *d, char *pReceBuffer )
{
	...
	g_World.m_Channel[nUserChannel].m_RoomList[nUserRoomNo].m_CPokerEngine.CreateGame( 52, 1 );
	...
	CRoomList *l_CRoomList;
	l_CRoomList = &g_World.m_Channel[nUserChannel].m_RoomList[nUserRoomNo];


	DTRACE("PokerBtnSet - 1");
	l_CRoomList->m_CPokerEngine.PokerBtnSet( nUserChannel, nUserRoomNo, nMasterSlot );
	l_CRoomList->m_CPokerEngine.PokerCalcuRaceMoney( nUserChannel, nUserRoomNo, nMasterSlot );
	...
	for ( i = 0; i < MAX_ROOM_IN; i++ ) {
		...
		if ( g_World.m_Channel[nUserChannel].m_RoomList[nUserRoomNo].m_User[i] != NULL  )
		{
			...
			memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
			nSendPacketLen = NGFrameBadukiStart( g_ClientSendBuf, pTempD, nRoomMasterUniqNo, nBbangJjangUniqNo, nPrizeDecide, nPrizeNo, nMasterSlot );
			SEND_TO_Q(g_ClientSendBuf, pTempD, nSendPacketLen);
			l_pCManager->SendOutput (pTempD);
		}
	}

	MyUserJokBoSend( nUserChannel, nUserRoomNo, d, TRUE );
...
void CardChange( CUserDescriptor *d, char *pReceBuffer )
{
...
#ifdef PACKETANALY
	sprintf( a_Log, "(CardChange_REQ) - UserIp : %s UserId : %s", d->m_Host, d->m_MyInfo.szUser_Id );
	LogFile (a_Log);
#endif
	...
	sUserNewCard l_sUserNewCard;

	l_CRoomList = &g_World.m_Channel[nChannel].m_RoomList[nRoomNo];


	memset( &l_sUserCardChange, 0x00, sizeof(l_sUserCardChange) );
	memset( &l_sUserNewCard, 0x00, sizeof(l_sUserNewCard) );

	memcpy( &l_sUserCardChange, pReceBuffer, sizeof(l_sUserCardChange) );

	if ( l_sUserCardChange.nChangeCardCnt < 0 ) {	//패스
		memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
		nSendPacketLen = NGFrameCardChange( g_ClientSendBuf, d->m_MyInfo.l_sUserInfo.nUserNo, &l_sUserNewCard, TRUE );
		BroadCastRoomInUser( g_ClientSendBuf, d, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d->m_nRoomInUserNo, nSendPacketLen );
	}
	else {
		l_CRoomList->m_CPokerEngine.CardChange( d, &l_sUserCardChange, &l_sUserNewCard );
		memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
		nSendPacketLen = NGFrameCardChange( g_ClientSendBuf, d->m_MyInfo.l_sUserInfo.nUserNo, &l_sUserNewCard, FALSE );
		BroadCastRoomInUser( g_ClientSendBuf, d, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d->m_nRoomInUserNo, nSendPacketLen );

		MyUserJokBoSend( nChannel, nRoomNo, d, FALSE );

	}

	d->m_bCardChange = TRUE;
	


	LogFile ("(CardChangeRep)\n");
...
{
#ifdef PACKETANALY
	sprintf( a_Log, "(CardChangeEnd_REQ) - UserIp : %s UserId : %s", d->m_Host, d->m_MyInfo.szUser_Id );
	LogFile (a_Log);
#endif


	d->m_bChangeState = FALSE;
	...
	for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
		if ( l_CRoomList->m_User[i] != NULL && l_CRoomList->m_User[i]->m_nGameJoin == 1 
			&& l_CRoomList->m_User[i]->m_bCardChange == FALSE ) 
		{

			int nNextUniqNo = g_World.FindNextUserUniqNo( nChannel, nRoomNo, d->m_nRoomInUserNo );

			int nSlot = g_World.FindSlot(  d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, nNextUniqNo );
			if ( nSlot >= 0 )
				l_CRoomList->m_User[nSlot]->m_bChangeState = TRUE;
			...
			l_CRoomList->m_User[nSlot]->m_bMyOrder = TRUE;

			memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
			nSendPacketLen = NGFrameChangeOkUser( g_ClientSendBuf, nNextUniqNo );
			BroadCastRoomInUser( g_ClientSendBuf, d, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d->m_nRoomInUserNo, nSendPacketLen );
	...
	l_CRoomList->m_CPokerEngine.NewRound( nChannel, nRoomNo );
	int nRoomMasterSlotNo = g_World.FindRoomMasterSlotNo( nChannel, nRoomNo );
	
	DTRACE("PokerBtnSet - 2, nChannel = %d, nRoomNo = %d, nRoomMasterSlotNo = %d, ", nChannel, nRoomNo, nRoomMasterSlotNo );
	BOOL bRe = l_CRoomList->m_CPokerEngine.PokerBtnSet( nChannel, nRoomNo, nRoomMasterSlotNo );
	
	DTRACE("CardChangeEnd - 111  bRe = %d", bRe);
	if ( bRe ) { 
		DTRACE("CardChangeEnd - 222");
		l_CRoomList->m_CPokerEngine.PokerCalcuRaceMoney( nChannel, nRoomNo, nRoomMasterSlotNo );

		memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);

		nSendPacketLen = NGFrameSetGameCurUser( g_ClientSendBuf, d, '8', nRoomMasterSlotNo );
		BroadCastRoomInUser( g_ClientSendBuf, d, nChannel, nRoomNo, d->m_nRoomInUserNo, nSendPacketLen );
	}

	DTRACE("CardChangeEnd - 333");

	LogFile ("(CardChangeEndRep)\n");
		...
		if ( nRe == -1 ) {
			...
			for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
				...
				if ( l_CRoomList->m_User[i]->m_sGameCurUser.bDie == FALSE )
					EndGameProc( l_CRoomList->m_User[i], '8', 1 );	
			}
		}
		else {
	...
	if ( nNextSlotNo < 0 ) {
		...
		if ( nRe == -1 ) {
			...
			if ( bRe ) { 
				...
				if ( l_CRoomList->m_nRound < 3 ) {

					int nRoomMasterUniqUserNo = g_World.FindRoomMasterUniNo( nChannel, nRoomNo );

					memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
					nSendPacketLen = NGFrameChangeOkAll( g_ClientSendBuf, d, l_CRoomList->m_nRound, cUserSelect );
					BroadCastRoomInUser( g_ClientSendBuf, d, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d->m_nRoomInUserNo, nSendPacketLen );

					int nSlot = g_World.FindSlot(  d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, nRoomMasterUniqUserNo );
					if ( nSlot >= 0 )
						l_CRoomList->m_User[nSlot]->m_bChangeState = TRUE;
					...
					memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
					nSendPacketLen = NGFrameChangeOkUser( g_ClientSendBuf, nRoomMasterUniqUserNo );
					BroadCastRoomInUser( g_ClientSendBuf, d, d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d->m_nRoomInUserNo, nSendPacketLen );
...
void UserChange( CUserDescriptor *d, char *pReceBuffer )
{
	...
	BOOL bReTempCardPack = FALSE;
	int nTempCardPackSlot = 0;

	if ( bRe == FALSE )
	{
		for ( i = 0; i < l_CRoomList->m_CPokerEngine.m_pChangeCardPack.m_nTop; i++ )
		{
			if ( l_CRoomList->m_CPokerEngine.m_pChangeCardPack.m_pChangeCard[i] == l_sUserChange.bChangeCardNo )
			{
				nCardPackSlot = i;
				bReTempCardPack = TRUE;
				break;
			}
		}
	}
	...
	if ( bRe == FALSE && bReTempCardPack == FALSE )
	{
		memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
	...
	else	// 바꾼 카드팩에서 유저의 카드를 교환한다.
		l_CRoomList->m_CPokerEngine.m_pChangeCardPack.m_pChangeCard[nTempCardPackSlot] = l_sUserChange.bUerCardNo;


	l_sUserChange.nUserUniqNo = d->m_MyInfo.l_sUserInfo.nUserNo;
	...
	MyUserJokBoSend( d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d, TRUE );
...
void MyUserJokBoSend( int nChannel, int nRoomNo, CUserDescriptor *d, BOOL bAll )
{
	CRoomList *l_CRoomList;
	int nSendPacketLen = 0;
	...
	{
		memset( g_ClientSendBuf, 0x00, NORMAL_SEND_BUFFER);
		nSendPacketLen = NGFrameMyUserJokBo( g_ClientSendBuf, d );
		SEND_TO_Q(g_ClientSendBuf, d, nSendPacketLen);
		l_pCManager->SendOutput (d);
	}
	...
	//MyUserJokBoSend( d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo, d, FALSE );
...
BigInt JackPotCalcu( CRoomList *l_CRoomList, int nMasterSlotNo )
{
	...
	for ( int k = 0; k < JACKPOT_CNT; k++ ) {
		if ( g_sJackPotInfo[k].biJackPotRoomMoney == l_CRoomList->l_sRoomList.l_sRoomInfo.biRoomMoney ) {
			biUserJackPotMoney = (float)g_sJackPotInfo[k].biJackPotMoney * ((float)nJackPotPerCent / 100);
...
void EndDrawGame( CUserDescriptor *d, char cUserSelect, int nChannel, int nRoomNo )
{
	...
	memset( bWinner, 0x00, sizeof(bWinner) );
	...
	int nMyRealBetMoney = 0;
	int nOtherRealBetMoney = 0;

	for ( i = 0 ; i < MAX_ROOM_IN ; i++ )
	{
		if ( bWinner[i] == FALSE || l_CRoomList->m_User[i] == NULL || l_CRoomList->m_User[i]->m_nGameJoin == 0 ) continue;

		nMyRealBetMoney = l_CRoomList->m_User[i]->m_sUserDBUpdateInfo.m_biRaceMoney - l_CRoomList->m_User[i]->m_biSideMoney;
		nOtherRealBetMoney = l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney - l_CRoomList->m_User[nMasterSlot]->m_biSideMoney;

		if ( nMyRealBetMoney < nOtherRealBetMoney )
			nMasterSlot = i;
	}

	DTRACE( "공동우승자중 우승자 : %s", l_CRoomList->m_User[nMasterSlot]->m_MyInfo.l_sUserInfo.szNick_Name );


	BigInt biCallBackTotalMoney = 0; 
	BigInt biCallBackMoney = 0; 
	...
	for ( int nSideTemp = 0; nSideTemp < MAX_ROOM_IN; nSideTemp++ ) {
		...
		if ( nMasterSlot != nSideTemp ) {
			...
			int nMyRaceRealMoney = l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney
				- l_CRoomList->m_User[nMasterSlot]->m_biSideMoney;
			...
			if ( nOtherRaceRealMoney > nMyRaceRealMoney ) {
				...
				DTRACE("(방:%d) %s유저가 올인을해서 %s유저에게 돌려준다! 돌려줄머니 : %I64u, 돌려준유저결과 - 총돈 : %I64u, 총배팅돈 : %I64u",
					l_CRoomList->m_User[nMasterSlot]->m_nRoomNo,
					l_CRoomList->m_User[nMasterSlot]->m_MyInfo.l_sUserInfo.szNick_Name,
					l_CRoomList->m_User[nSideTemp]->m_MyInfo.l_sUserInfo.szNick_Name,
					biCallBackMoney, 
	...
	biCallBackTotalMoney += l_CRoomList->m_User[nMasterSlot]->m_biSideMoney;


	BigInt biMyBetTotMoney = l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney;

	l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney = 
		l_CRoomList->m_biRoomCurMoney - biCallBackTotalMoney - 
		( l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney - l_CRoomList->m_User[nMasterSlot]->m_biSideMoney );

/*
	BigInt biSystemMoney = (float)(l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney + biMyBetTotMoney )
							* ((float)SYSTEM_MONEY_RATE / 100);
*/

    float Rate_Sys = g_pDbQ->g_ChonSystemMoneyRate;

	BigInt biSystemMoney = (float)(l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney + biMyBetTotMoney )
								* ((float)(Rate_Sys/100.0));


	BigInt biPureTotalPanMoney = ( l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney + biMyBetTotMoney ) - biSystemMoney;
	BigInt biPerWinMoney = (float)biPureTotalPanMoney / (float)nWinnerCnt;
	BigInt biPerSystemMoney = (float)biSystemMoney / (float)nWinnerCnt;

	BigInt biPurePanMoney = l_CRoomList->m_User[nMasterSlot]->m_sUserDBUpdateInfo.m_biRaceMoney + biMyBetTotMoney;
	...
	for ( i = 0; i < MAX_ROOM_IN; i++ )
	{
		if ( bWinner[i] == TRUE )
		{
			...
			BigInt biPerLastMoney = biPerWinMoney - biMyBetTotMoney;
			...
			l_CRoomList->m_User[i]->m_sUserDBUpdateInfo.biRealRaceMoney = biPerLastMoney;
			l_CRoomList->m_User[i]->m_biTempRealRaceMoney = biPerLastMoney;
			l_CRoomList->m_User[i]->m_sUserDBUpdateInfo.m_biRaceMoney = biPerLastMoney;

			l_CRoomList->m_User[i]->m_MyInfo.l_sUserInfo.biUserMoney += biPerLastMoney + biMyBetTotMoney;
			
			l_CRoomList->m_User[i]->m_sUserDBUpdateInfo.biSystemMoney = biPerSystemMoney;	

			DTRACE("(방:%d) 승리자(%s)의 머니 계산 - 실제판머니(승리) : %I64u, 유저가 딴순머니 : %I64u,유저가 건 순베팅머니 : %I64u, 시스템머니 : %I64u",
				l_CRoomList->m_User[i]->m_nRoomNo,
				l_CRoomList->m_User[i]->m_MyInfo.l_sUserInfo.szNick_Name,
				biPurePanMoney,
				l_CRoomList->m_User[i]->m_sUserDBUpdateInfo.m_biRaceMoney,
				biMyBetTotMoney,
				biPerSystemMoney );
		}
	}
	...
	BigInt biUserJackPotMoney = 0;
	int nWinnerUnqiNo[MAX_ROOM_IN];

	memset( nWinnerUnqiNo, -1, sizeof(nWinnerUnqiNo) );
	...
	for ( i = 0; i < MAX_ROOM_IN; i++ ) {
		...
		if ( bWinner[i] ) {
			...
			nWinnerUnqiNo[i] = l_CRoomList->m_User[i]->m_MyInfo.l_sUserInfo.nUserNo;
		}
		else 
	...
	nSendPacketLen = NGFrameEndGame( g_ClientSendBuf, nChannel, nRoomNo, nMasterSlot, nBeforeRoomMasterRoomInUserNo, 
		d, cUserSelect, l_CRoomList->m_User[nMasterSlot]->m_CPokerEvalution.m_nJackPot, biUserJackPotMoney, nWinnerUnqiNo, TRUE );
...
void EndGameProc( CUserDescriptor *d, char cUserSelect, int nEndDivi )
{
	...
	int nMasterSlotNo = l_CRoomList->m_CPokerEngine.GameRanking( d->m_MyInfo.l_sUserInfo.nChannel, d->m_nRoomNo );

	if ( nEndDivi != 2 && nMasterSlotNo < 0 )
	{
		EndDrawGame( d, cUserSelect, nChannel, nRoomNo );
		return;
	}
	...
	if ( nEndDivi == 0 || nEndDivi == 1 ) 
	{
		...
		TRACE("BB(방:%d) 승리자(%s)의 머니 계산 - 판머니(승리) : %I64u, 유저가 딴순머니 : %I64u,유저가건 순 베팅머니 : %I64u \n",
			l_CRoomList->m_User[nMasterSlotNo]->m_nRoomNo,
			l_CRoomList->m_User[nMasterSlotNo]->m_MyInfo.l_sUserInfo.szNick_Name,
		...
		TRACE("AA(방:%d) 딴순머니 : %I64u, SYSTEM MONEY : %I64u \n",
			l_CRoomList->m_User[nMasterSlotNo]->m_nRoomNo,
		...
		DTRACE("(방:%d) 시스머니뺀딴순머니 : %I64u, RoomMoney : %I64u", 
			l_CRoomList->m_User[nMasterSlotNo]->m_nRoomNo,
			l_CRoomList->m_User[nMasterSlotNo]->m_sUserDBUpdateInfo.m_biRaceMoney, l_CRoomList->l_sRoomList.l_sRoomInfo.biRoomMoney );
	...
	if ( nEndDivi == 0 ) {
		...
		nSendPacketLen = NGFrameEndGame( g_ClientSendBuf, nChannel, nRoomNo, nMasterSlotNo, nBeforeRoomMasterRoomInUserNo, 
			d, cUserSelect, l_CRoomList->m_User[nMasterSlotNo]->m_CPokerEvalution.m_nJackPot, biUserJackPotMoney, nWinnerUnqiNo );
...
void BroadCastChannel( char *pSendBuff, int nChannel, int nSendPacketLen, bool bViewUserPageEx, CUserDescriptor *d )
{
	...
	if ( bViewUserPageEx == false ) {
		...
		if ( d->m_nUserPage == pUserList->m_User->m_nUserPage ) continue; 

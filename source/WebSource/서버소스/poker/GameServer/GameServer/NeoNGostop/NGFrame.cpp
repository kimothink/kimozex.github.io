...
int NGFrameWaitInput( char *pBuffer, CUserDescriptor *d )
{
	...
	nTemp =  nIndex; //유저리스트의 정보를 넣기 앞서서 몇명인지를 넣어야 하므로. 자리를 비워둔다.^
	nIndex += sizeof(ush_int); 
...
int NGFrameStartGame( char *pBuffer, CUserDescriptor *d, int nRoomMasterUniqNo, int nBbangJjangUniqNo, int nPrizeDecide, int nPrizeNo ) 
{
	...
	memset( &l_sStartTotUserCard, 0x00, sizeof(l_sStartTotUserCard) );

	l_sStartTotUserCard.nRoomMasterUniqNo = nRoomMasterUniqNo;
	l_sStartTotUserCard.nBbanJjangUniqNo = nBbangJjangUniqNo;
	l_sStartTotUserCard.biRoomCurMoney = l_CRoomList->m_biRoomCurMoney;
	for ( int i = 0; i < JACKPOT_CNT; i++ ) {
		if ( g_sJackPotInfo[i].biJackPotRoomMoney == l_CRoomList->l_sRoomList.l_sRoomInfo.biRoomMoney ) {
			l_sStartTotUserCard.biJackPotMoney = g_sJackPotInfo[i].biJackPotMoney;
			break;
		}
	}	
	l_sStartTotUserCard.bChoiceRoom = l_CRoomList->l_sRoomList.l_sRoomInfo.bChoiceRoom;

	l_sStartTotUserCard.biJackPotMoney = g_sJackPotInfo[0].biJackPotMoney; //JackPot Money
	...
	for ( j = 0; j < MAX_ROOM_IN; j++ ) 
	{
		...
		l_sStartTotUserCard.l_sStartPerUserCard[j].nUserUniqNo = l_CRoomList->m_User[nTemp]->m_MyInfo.l_sUserInfo.nUserNo;
		l_sStartTotUserCard.l_sStartPerUserCard[j].nCardCnt = l_CRoomList->m_User[nTemp]->m_nCardCnt;

		for ( int k = 0; k < l_CRoomList->m_User[nTemp]->m_nCardCnt; k++  ) {
			l_sStartTotUserCard.l_sStartPerUserCard[j].byCardNo[k] =  l_CRoomList->m_User[nTemp]->m_nUserCard[k];
		}
	}
	//////////////////////////////////////////////////////////////////////////
...
//	l_sGameStart.l_sStartUserCard.biJackPotMoney = g_sJackPotInfo[0].biJackPotMoney;
	...
	memcpy( &pBuffer[nIndex], &l_sStartTotUserCard, sizeof(l_sStartTotUserCard) );
...
int NGFrameViewPage( char *pBuffer, int nUserChannel, int nUserReqPage )
{
	...
	for ( int i = nInit; i < nFinal; i++ ) {	//방정보와 방안의 유저정보를 가져온다.
...
int NGFrameInitUserCard( char *pBuffer, CUserDescriptor *d )
{
	int nIndex = SC_HEADER_SIZE;

	sInitUserCard l_sInitUserCard;
	memset( &l_sInitUserCard, 0x00, sizeof(l_sInitUserCard) );

	l_sInitUserCard.nUserUniqNo = d->m_MyInfo.l_sUserInfo.nUserNo;
	memcpy( &l_sInitUserCard.nUserCard, d->m_nUserCard, sizeof(d->m_nUserCard) );

	memcpy( &pBuffer[nIndex], &l_sInitUserCard, sizeof(l_sInitUserCard) );
	nIndex += sizeof(l_sInitUserCard);

	NGFrameHeader( pBuffer, nIndex, NGSC_INITUSERCARD, ERROR_NOT );
...
int NGFrameEndGame( char *pBuffer, int nChannel, int nRoomNo, int nMasterSlotNo, int nBeforeRoomMasterRoomInUserNo, CUserDescriptor *d, 
				   char cUserSelect, ush_int nJackPot, BigInt biUserJackPotMoney )
{
	//nJackPot = 1;

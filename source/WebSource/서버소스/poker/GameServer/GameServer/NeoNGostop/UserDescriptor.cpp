...
void CUserDescriptor::Evalution()
{
	...
	if (m_nCardCnt <= 3) // 4장째에서 부터 해야한다.
		m_CPokerEvalution.Calcul(m_nUserCard + 2, 1);
	else if (m_nCardCnt <= 6)
		m_CPokerEvalution.Calcul(m_nUserCard + 2, m_nCardCnt - 2 );
	else
		m_CPokerEvalution.Calcul(m_nUserCard, 7);

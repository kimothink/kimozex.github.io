...
/*
float   g_ChonSystemMoneyRate=0.0;
float   g_ChonSystemJackPotRate=0.0;
*/
	try
	{
...
bool CDbQ::GetUserInfo( CUserDescriptor *d )
{
	...
	try
	{
		...
		l_AdoRS.m_pCmd->CommandText = "uspG_newPokerLogin";
...
bool CDbQ::UserUpdate( CUserDescriptor *d )
{
	...
	try
	{
		pCmd.CreateInstance(__uuidof(Command));
		pCmd->ActiveConnection = m_AdoDB.m_pConnection;
		pCmd->CommandText = "uspG_newpoker7_Result";
		...
		pCmd->Parameters->Append(pCmd->CreateParameter("@V_JackPotDivi"  ,adInteger,adParamInput,sizeof(VARIANT),m_v_JackPotDivi)); 
		...
		pCmd->Parameters->Append(pCmd->CreateParameter("@V_JackPotMoney"  ,adVarChar,adParamInput,sizeof(char) * MAX_GAMESTRMONEYLEN,m_v_UserJackPotMoney)); 
...
void ThreadDbFunc(CDbQ *pQ)
{
	...
	while (true) {
		...
		bRe = pQ->m_CJackPotQ.GetFromQ( &l_sJackPotQ );

		if ( bRe )
			pQ->JackPotUpdate( &l_sJackPotQ );

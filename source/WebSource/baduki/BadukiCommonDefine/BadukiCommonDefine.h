...
typedef struct tagUserTotalCardInfo
{
	...
	typedef struct tagEndGame
	{
		...
		BYTE szFixJokboCard[MAX_ROOM_IN][8];
		BOOL bDrawGame;
		int nDrawWinnerUniq[MAX_ROOM_IN];
	} sEndGame;
...
typedef struct tagUserCardChange	//카드를 바꾼다.
{
	int  nCardSlot[4];				//0이면 안바꾼다.
	BYTE nCardNo[4];				//실제 카드 번호
	int  nChangeCardCnt;				//바꿀 카드수
} sUserCardChange;

typedef struct tagUserNewCard		//카드를 바꾼다.
{
	int nCardSlot[4];				//0이면 안바꾼다.
	BYTE nNewCardNo[4];				//실제 카드 번호가
} sUserNewCard;

typedef struct tagUserNewCardPacket		//카드를 바꾼다.
{
	BOOL bPass;
	int nUniqUserNo;				//유저 고유 번호
...
#define NGSC_CHANGECARD				0x27		//카드를 바꾼다.
#define NGSC_CHANGEOKALL			0x28		//아침, 점심 , 저녁을 택한다.
#define NGSC_CHANGEOKUSER			0x29		//특정 유저가 카드를 바꿀수 있음을 알린다.
#define NGSC_ALLUSERJOKBO			0x40		//유저 족보 모두 보낸다.
#define NGSC_USERCHANGE				0x41
...
#define NGCS_CARDCHANGE				10		//유저가 카드를 바꾼다.
#define NGCS_CARDCHANGEEND			11		//카드 다 바꾸었다.
#define NGCS_USERCHANGE				12		//유저가 임의로 카드 바꾸기.
...
*/
	...
	//잭팟 적립 비율.
	#define	JACKPOT_STIFUL			20
	#define JACKPOT_ROTIFUL			30

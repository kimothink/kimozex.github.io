...
void CTextList::AddText(STextInfo *pTextInfo)
{
    ...
    sTextInfo.nStyle = pTextInfo->nStyle;       // 글자 스타일(0=보통, 1=굵게)
    sTextInfo.rgbColor = pTextInfo->rgbColor;   // 글자 색
    nStrLen = strlen(pTextInfo->szText);        // 글자
    nStartIdx = 0;
    ...
    if ( nStrLen <= 0 && nStrLen > 255 ) {
        DTRACE2("[%s] CTextList::AddText - 없거나 비정상적인 문자열 : nStrLen=%d", g_szUser_ID, nStrLen);
        return;
    }

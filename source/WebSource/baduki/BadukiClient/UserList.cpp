...
CGameUserInfo::Init()
{
    ...
    memset(&m_sNewCard,         0x00, sizeof( m_sNewCard ));
}
...
// ham 2006.01.25
void CGameUserInfo::InitOneCard( int nIdx )
{
    if ( nIdx >= 0 && nIdx < MAX_CARD ) {
        m_sUserCardInfo[nIdx].NewPos        = g_pCMainFrame->SetPoint(INIT_CARD_POS_X, INIT_CARD_POS_Y);
		m_sUserCardInfo[nIdx].StartPos      = g_pCMainFrame->SetPoint(INIT_CARD_POS_X, INIT_CARD_POS_Y);
		m_sUserCardInfo[nIdx].Pos           = g_pCMainFrame->SetPoint(INIT_CARD_POS_X, INIT_CARD_POS_Y);
		m_sUserCardInfo[nIdx].nAniCount     = 0;
		m_sUserCardInfo[nIdx].nAniMaxCount  = -1;
		m_sUserCardInfo[nIdx].nCardNo       = -1;
		m_sUserCardInfo[nIdx].ctCardType    = PACK;
		m_byUserRealCardNo[nIdx]            = -1;
    }
}

// ham 2006.01.25
void CGameUserInfo::InitOneCard( sUserCardInfo *pCard )
{
    for ( int i = 0; i < MAX_CARD; i++ ) {
        if ( &m_sUserCardInfo[i] == pCard ) {
            InitOneCard( i );
            return;
        }
    }
...
void CGameUserList::UserCardPosRefresh( int nUserArrayNo )
{
	for ( int i = 0; i < m_GameUserInfo[nUserArrayNo].m_nCardCnt; i++ ) {
		m_GameUserInfo[nUserArrayNo].m_sUserCardInfo[i].Pos.x = g_pCMainFrame->m_sRoomInUserPosition[nUserArrayNo].pCardStartPos.x + i * CARD_DISTANCE_A; 
	}
}

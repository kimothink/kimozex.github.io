...
#include "..\BadukiCommonDefine\serverclientcommon.h"
...
CMainFrame::CMainFrame()
{
	...
	m_bBeforeFull = FALSE;
    m_bPassRoomChk = FALSE;         // [HAM+] 2005.08.19 [HAM-]
    m_bPassDlg = FALSE;
    m_bShowRoomTitleList = FALSE;
    m_bMoveHiddenCard = FALSE;
    m_ChonHiddenMode  = FALSE;
    m_bChgUpCard = FALSE;
    m_bChgDownCard = FALSE;
    m_bMyChgCard = FALSE;
    m_bMyChgCardTimePassed = FALSE;
    m_bChgMyJokBo = FALSE;
    m_bDrawGame = FALSE;
    ...
    memset(&m_sTimeMode,             0x00, sizeof( m_sTimeMode));
    memset( m_sChgCardInfo,          0x00, sizeof( m_sChgCardInfo ));
    memset(&m_sNextUserInfo,         0x00, sizeof( m_sNextUserInfo ));
    memset(&m_sTempMyUserJokBo,      0x00, sizeof( m_sTempMyUserJokBo ));
    memset( m_nDrawWinner,           0x00, sizeof( m_nDrawWinner ));
    memset(&m_sStartUserSync,        0x00, sizeof( m_sStartUserSync ));
    ...
    m_SPR_ChodaeOX = NULL;
    m_SPR_BangJang = NULL;
	m_SPR_TimeModeBig = NULL;
	m_SPR_TimeModeSmall = NULL;
    m_SPR_ChgCardNum = NULL;
    m_SPR_ChgCardPass = NULL;
    m_SPR_SelCardKey = NULL;
    m_SPR_BetBtnKey = NULL;
    m_SPR_ChgCardMsg = NULL;
    m_SPR_Arrow = NULL;
    ...
    m_ChgCardChgBTN = NULL;
    m_ChgCardPassBTN = NULL;
	...
	m_sRoomInUserPosition[0].pJokboPos = SetPoint( m_sRoomInUserPosition[0].pCardInfo.x + 111, m_sRoomInUserPosition[0].pCardInfo.y + 138 );
    ...
    // 경품 당첨 도장
    m_sRoomInUserPosition[0].pGoodsDojangPos = SetPoint( m_sRoomInUserPosition[0].pCardStartPos.x + 104, m_sRoomInUserPosition[0].pCardStartPos.y - 49 );
    m_sRoomInUserPosition[1].pGoodsDojangPos = SetPoint( m_sRoomInUserPosition[1].pCardStartPos.x + 104, m_sRoomInUserPosition[1].pCardStartPos.y - 49 );
    m_sRoomInUserPosition[2].pGoodsDojangPos = SetPoint( m_sRoomInUserPosition[2].pCardStartPos.x + 104, m_sRoomInUserPosition[2].pCardStartPos.y - 49 );
    m_sRoomInUserPosition[3].pGoodsDojangPos = SetPoint( m_sRoomInUserPosition[3].pCardStartPos.x -  38, m_sRoomInUserPosition[3].pCardStartPos.y - 49 );
    m_sRoomInUserPosition[4].pGoodsDojangPos = SetPoint( m_sRoomInUserPosition[4].pCardStartPos.x -  38, m_sRoomInUserPosition[4].pCardStartPos.y - 49 );
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//넘어온 인자확인
	if( __argc != 9 ) {       //실행파일 서버넘버, 아이디
		::MessageBox( hWnd, "웹에서 실행해주세요.", "로우바둑이", MB_OK );
        ExitGame();
		return 0;
	}
...
void CMainFrame::NetWorkThreaOnGameInit()
{
    ...
    AllChgCardBtnDisable();     // 모든 카드 바꾸기 버튼 사용 금지
    ...
    //네트워크 스레드에서 초기화를 기다리고 있다.
    //DTRACE2("네트워크 쓰레드에서 초기화");
...
void CMainFrame::AnimationUpdate()
{
	...
	if ( m_bUserCallTime ) {
		...
		if ( m_nUserCallTime > m_nUserCallDefineTime ) {
			...
			if ( m_nUserCallSlotNo == 0 ) {
                // 카드 바꾸기 모드에서 타이머가 다 되면 "패스" 처리한다.
                if ( m_sTimeMode.nTime >= TTIME_MORNING && m_sTimeMode.nMode == TMODE_CHGCARD ) {
                    m_bMyChgCardTimePassed = TRUE;  // 내 카드 바꾸는 시간이 지나갔음
                    AllChgCardBtnDisable();
                    ProcessPassBTN();
                }

                // 베팅모드에서 타이머가 다 되면 "다이" 처리한다.
                else {
                    AllCallBtnUp();
	...
	if ( m_bEnddingShow ) {
		...
		if ( m_bStartInit ) {
			...
			for ( int i = 0; i < g_CGameUserList.m_GameUserInfo[0].m_nCardCnt; i++ ) {
				g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].Pos.x = m_sRoomInUserPosition[0].pCardStartPos.x + ( i * CARD_DISTANCE_A );
		...
		if ( m_nEndingShowTime == 0 ) {
			...
			for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
					 ...
					 g_CGameUserList.m_GameUserInfo[i].m_biPureUserWinLoseMoney > 0 ) 
				{
					...
					if ( g_CGameUserList.m_GameUserInfo[i].m_bDie ) {
						sprintf( pTemp, "◎ %s님 다이", g_CGameUserList.m_GameUserInfo[i].m_sUserInfo.szNick_Name );
                        SetGameChatText( pTemp , RGB( 100, 255, 50 ) );
					}
                    else if ( g_CGameUserList.m_GameUserInfo[0].m_bDie != TRUE && g_CGameUserList.m_GameUserInfo[i].m_nCardCnt >= 4 ) {
			 ...
			 g_CGameUserList.m_GameUserInfo[m_nEndingUserCurSlot].m_bDie == FALSE ) 
		{
			if ( m_nEndingUserCurTime % 25 == 0 ) {     //카드를 확 핀다.
				for ( int i = 0; i < g_CGameUserList.m_GameUserInfo[m_nEndingUserCurSlot].m_nCardCnt; i++ ) {
					...
					g_CGameUserList.m_GameUserInfo[m_nEndingUserCurSlot].m_sUserCardInfo[i].Pos.x = m_sRoomInUserPosition[m_nEndingUserCurSlot].pCardStartPos.x + ( i * CARD_DISTANCE_A );
				}
    ...
    //-----------------------------------------------------------
    // 초기카드를 다 나누어 준 다음에 게임이 진행되도록 설정한다.
    //-----------------------------------------------------------

    if ( m_sStartUserSync.bInsertInitCard == TRUE && m_CAniMation.GetAniCount() < 0 ) {
        SetUserCall( m_sStartUserSync.l_sCurUser, m_sStartUserSync.nSlotNo );
        memset( &m_sStartUserSync, 0x00, sizeof( m_sStartUserSync ));
    }
	...
	for( i = 0; i <= m_CAniMation.GetAniCount(); i++ ) // 카드 이동 애니메이션 처리
	{
		...
		if( CheckPosition(card) ) {
            ...
            if ( card->ctCardType == CHGCARD ) {
                g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].InitOneCard( card );
            }
            else {
            ...
            // 게임 시작하자마자 내 카드를 다 받았으면 내 족보를 갱신한다.
            if ( m_sTimeMode.nTime < TTIME_MORNING && m_bChgMyJokBo == TRUE ) {
                if ( GetUserCardCnt( 0 ) >= 4 ) {
                    UpdateMyJokBo();
                }
            }

            // 바꿀 카드가 다 위로 이동했으면 새 카드를 넣는다.

            if ( m_bChgUpCard == TRUE && m_CAniMation.GetAniCount() < 0 ) {
                m_bChgUpCard = FALSE;
                AddNewCard();
            }
		}
    ...
    // 교환한 카드가 지금 막 유저에게 도착했고, 그 카드가 자신의 카드라면,
    // 카드를 교환했다고 서버에게 알린다.

    if ( m_bMyChgCard == TRUE && m_bChgDownCard == TRUE &&  m_CAniMation.GetAniCount() < 0 )
    {
        m_bChgDownCard = FALSE;
        m_bMyChgCard = FALSE;

        if ( m_nUserCallSlotNo == 0 ) {
            g_pClient->ChangeUserCardEnd();
            UpdateMyJokBo();
        }
    }

    // 아침 이전(카드 바꾸기 이전)일 때, 또는 아침 이후 이동중인 카드가 없을 때

    if ( m_sNextUserInfo.bChange == TRUE && m_CAniMation.GetAniCount() < 0 ) {
        m_sNextUserInfo.bChange = FALSE;
        SetGameCurUser();
    }
}
...
void CMainFrame::LoadData()
{
    ...
    m_SPR_WaitBase      = LoadSprite(".\\image\\lobby\\waitbase.bmp",           1024, 768,   1 );
    ...
    m_SPR_MakeRoom      = LoadSprite(".\\image\\lobby\\dlg_makeroom.spr",        325, 315,   1 );
    ...
    m_SPR_GameBase      = LoadSprite(".\\image\\game\\gamebase.tga",          1024, 768,  1 );
    ...
    m_SPR_WindowDraw    = LoadSprite(".\\image\\game\\dlg_draw.spr",           304, 200,  1 );
	...
	m_SPR_TimeModeBig	= LoadSprite(".\\image\\game\\time_mode_big.spr",      257, 125,  3 );
	m_SPR_TimeModeSmall = LoadSprite(".\\image\\game\\time_mode_small.spr",     65,  26,  6 );
    m_SPR_ChgCardNum    = LoadSprite(".\\image\\game\\chgcard_num.spr",         21,  30,  4 );
    m_SPR_ChgCardPass   = LoadSprite(".\\image\\game\\chgcard_pass.spr",        70,  35,  1 );
    m_SPR_SelCardKey    = LoadSprite(".\\image\\game\\key_selcard.spr",         20,  17,  4 );
    m_SPR_BetBtnKey     = LoadSprite(".\\image\\game\\key_betbtn.spr",         210, 154,  1 );
    m_SPR_ChgCardMsg    = LoadSprite(".\\image\\game\\chgcard_msg.spr",        210,  26,  3 );
...
*/
    ...
    m_ChgCardChgBTN  = LoadButton(".\\image\\game\\btn_chgcard_chg.spr",  761, 612, 70, 43 );
    m_ChgCardPassBTN = LoadButton(".\\image\\game\\btn_chgcard_pass.spr", 871, 612, 70, 43 );
...
void CMainFrame::SetColorKey()
{
    ...
    if ( m_SPR_WindowDraw       != NULL ) m_SPR_WindowDraw->SetColorKey( dwColorKey );
	...
	if ( m_SPR_TimeModeBig	!= NULL ) m_SPR_TimeModeBig->SetColorKey( dwColorKey );
	if ( m_SPR_TimeModeSmall!= NULL ) m_SPR_TimeModeSmall->SetColorKey( dwColorKey );
    if ( m_SPR_ChgCardNum   != NULL ) m_SPR_ChgCardNum->SetColorKey( dwColorKey );
    if ( m_SPR_ChgCardPass  != NULL ) m_SPR_ChgCardPass->SetColorKey( dwColorKey );
    if ( m_SPR_SelCardKey   != NULL ) m_SPR_SelCardKey->SetColorKey( dwColorKey );
    if ( m_SPR_BetBtnKey    != NULL ) m_SPR_BetBtnKey->SetColorKey( dwColorKey );
    if ( m_SPR_ChgCardMsg   != NULL ) m_SPR_ChgCardMsg->SetColorKey( dwColorKey );
    ...
    if ( m_ChgCardChgBTN    != NULL ) m_ChgCardChgBTN->SetColorKey( dwColorKey );
    if ( m_ChgCardPassBTN   != NULL ) m_ChgCardPassBTN->SetColorKey( dwColorKey );
...
void CMainFrame::ReleaseData()
{
	...
	SAFE_DELETE( m_SPR_TimeModeBig );
	SAFE_DELETE( m_SPR_TimeModeSmall );
    SAFE_DELETE( m_SPR_ChgCardNum );
    SAFE_DELETE( m_SPR_ChgCardPass );
    SAFE_DELETE( m_SPR_SelCardKey );
    SAFE_DELETE( m_SPR_BetBtnKey );
    SAFE_DELETE( m_SPR_ChgCardMsg );
    ...
    SAFE_DELETE( m_ChgCardChgBTN );
    SAFE_DELETE( m_ChgCardPassBTN );
		...
		if ( g_bMyWin == FALSE ) {
            ...
            for ( int nQ = 0; nQ < 4; nQ++ ) {
				if ( nCardNo == g_CGameUserList.m_GameUserInfo[g_nWinnerSlotNo].m_szFixJokboCard[nQ] ) {
					PutCardImage( 386 + i * 57, 345, nCardNo );
					bRe = TRUE;
					break;
				}
			}
            ...
            if ( bRe == FALSE ) {
                PutCardImage( 386 + i * 57, 345, nCardNo ); // ham 2006.01.31
            }
		}
        else {
            PutCardImage( 386 + i * 57, 345, nCardNo );
...
//========================================================================
// 여러명이 동시에 우승했을 경우를 출력한다.
//========================================================================

void CMainFrame::RenderDrawWinner()
{
    if ( !m_bDrawGame ) {
        return;
    }
    ...
    for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
        if ( m_nDrawWinner[i] == 1 ) {
            PutSprite( m_SPR_WinnerMark,
                       m_sRoomInUserPosition[i].pWinnerPos.x,
                       m_sRoomInUserPosition[i].pWinnerPos.y,
                       m_nWinnerMarkPos );
        }
    }

    // 무승부 창 출력
    PutSprite( m_SPR_WindowDraw, 360, 284 );
}
...
void CMainFrame::SetUserCall( sGameCurUser asGameCurUser, int nUserArrayNo )
{
    ...
    m_nTemp = 0;
	m_nTimePos = 0;
    m_bMyChgCardTimePassed = FALSE;         // SerUserCall이 호출되는 타이밍이면 카드 바꾸는 시간이전이거나, 지났으므로 초기화.

    m_sTimeMode.nMode = TMODE_BET;          // ham 2006.01.24
	...
	if ( nUserArrayNo == 0  ) {
        ...
        */
        ...
        PlaySound(g_Sound[GS_MYTURN]);  // 자신의 차례가 되었다고 알린다.
	}
    else {
        g_biCallMoney = 0;
    }
...
void CMainFrame::RenderGameBase()
{
    ...
    if ( m_nUserCallSlotNo == 0 && m_sTimeMode.nTime >= TTIME_MORNING && m_sTimeMode.nTime <= TTIME_EVENING && m_sTimeMode.nMode == TMODE_CHGCARD ) {
        m_ChgCardChgBTN->DrawButton();  // 카드 바꾸기 버튼
        m_ChgCardPassBTN->DrawButton(); // 패스 버튼

        switch ( m_sTimeMode.nTime ) {
            case TTIME_MORNING:
                PutSprite( m_SPR_ChgCardMsg, 754, 673, 0 );
                break;

            case TTIME_AFTERNOON:
                PutSprite( m_SPR_ChgCardMsg, 754, 673, 1 );
                break;

            case TTIME_EVENING:
                PutSprite( m_SPR_ChgCardMsg, 754, 673, 2 );
                break;
        }
    }
    else {
        // 버튼 단축키 텍스트 출력
        PutSprite( m_SPR_BetBtnKey, 731, 557 );
    ...
    // 카드 바꾸기 단축키를 그린다.
    if ( m_sTimeMode.nTime >= TTIME_MORNING && m_sTimeMode.nMode == TMODE_CHGCARD && m_bMyChgCardTimePassed == FALSE ) {
        // (1)
        PutSprite(
            m_SPR_SelCardKey,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[0].Pos.x + 10,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[0].Pos.y + CARD_HEIGHT,
            0
        );
        ...
        PutSprite(
            m_SPR_SelCardKey,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[1].Pos.x + 10,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[1].Pos.y + CARD_HEIGHT,
            1
        );
        ...
        PutSprite(
            m_SPR_SelCardKey,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[2].Pos.x + 10,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[2].Pos.y + CARD_HEIGHT,
            2
        );
        ...
        PutSprite(
            m_SPR_SelCardKey,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[3].Pos.x + 10,
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[3].Pos.y + CARD_HEIGHT,
            3
        );
    }
    ...
    // 화면 중앙에 아침/점심/저녁 출력
    if ( m_sTimeMode.nTime >= TTIME_MORNING && m_sTimeMode.nTime <= TTIME_EVENING && m_sTimeMode.nMode == TMODE_CHGCARD ) {
        PutSprite( m_SPR_TimeModeBig, 383, 321, m_sTimeMode.nTime - 1 );
    }
    ...
    // 톨 슬라이딩 그리기
    RenderThrowChip();      // 유저가 버튼(쿼터, 하프 등)을 눌러서 던진 톨을 그린다.
    ...
    if ( m_bEnddingShow ) {
        if ( m_bDrawGame ) {
            RenderDrawWinner(); // 이긴 사람이 여러명인 경우
        }
        else {
            RenderWinner();     // 이긴 사람이 한 명인 경우
        }
    }
	...
	memset( m_szTempText, 0x00, sizeof(m_szTempText) );
	sprintf( m_szTempText , "로우바둑이[%d채널]" , g_szGameServerChannel + 1 );
	...
	sprintf( m_szTempText , "%d번방  방제 - 대박 닐리리~ 즐거운 게임한판!   [기본 : %d톨]" , g_sRoomInfo.nRoomNo + 1, g_sRoomInfo.biRoomMoney );	
...
void CMainFrame::NickSavePrint( char *pNickName, int x, int y, COLORREF rgbCol )
{
	...
	if ( strlen( pNickName ) > 10 ) {
        ...
        DrawText( "닐리리", x, y, rgbCol, FW_NORMAL, TA_LEFT );
	}
	else {
        DrawText( pNickName, x, y, rgbCol, FW_NORMAL, TA_LEFT );
        //DrawText( "test", x, y, rgbCol, FW_NORMAL, TA_LEFT );
	}
}
	...
	for ( i = 0; i < MAX_ROOM_IN; i++ ) {
		...
		for ( int j = 0; j < g_CGameUserList.m_GameUserInfo[nUserArrayNo].m_nCardCnt; j++ )
        {
			if ( g_CGameUserList.m_GameUserInfo[nUserArrayNo].m_sUserCardInfo[j].ctCardType == USERCARD ||
                 g_CGameUserList.m_GameUserInfo[nUserArrayNo].m_sUserCardInfo[j].ctCardType == CHGCARD)
            {
                // 이동중인 CHGCARD 타입의 카드는 이동중인 카드 출력할 때 출력되므로,
                // 유저 카드 찍을 부분에서는 이동중인 CHGCARD 카드는 찍지 않는다.
                
                if ( &(g_CGameUserList.m_GameUserInfo[nUserArrayNo].m_sUserCardInfo[j]) == m_CAniMation.GetCard( 0 )) {
                    continue;
                }
                ...
                if(m_sTimeMode.nTime ==TTIME_EVENING ) {
					int iMax=-1,iDelta;
					for(int u=0;u<4;u++) {
						iMax = (m_MyHiddenPosition[u]>iMax) ? m_MyHiddenPosition[u] : iMax;
					}	
					if(iMax>-1 ) {
						if(iMax==3)
							iDelta = 30;
							...
							m_SPR_Arrow->SetPos( g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.x+iDelta ,
												 m_CurHiddenYPos - 12);
				...
				if ( g_CGameUserList.m_GameUserInfo[nUserArrayNo].m_nCardCnt == 4 && 
...
void CMainFrame::RenderUserInfo()
{
	...
	for ( int i = 0; i < MAX_ROOM_IN; i++ ) {
		if(i==0)
			TRACE("m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt ==> %d \n",m_sChgCardInfo[i].l_sUserCardChange.nChangeCardCnt);
		...
		if ( g_CGameUserList.m_GameUserInfo[i].m_sUserInfo.nUserNo > 0 ) {
            ...
            */
            ...
            // 유저가 교환한 카드의 숫자를 출력한다.

            if ( m_sTimeMode.nTime >= TTIME_MORNING && 
                 m_sTimeMode.nTime <= TTIME_EVENING &&
                 m_sChgCardInfo[i].nStatus == 1 &&          // 카드 바꾸기가 완료된 유저
                 i != 0 )
            {
                // Pass
                if ( m_sChgCardInfo[i].l_sUserCardChange.nChangeCardCnt == -1 ||  m_sChgCardInfo[i].l_sUserCardChange.nChangeCardCnt == 0 ) {
                    PutSprite(
                        m_SPR_ChgCardPass,
                        m_sRoomInUserPosition[i].pCardStartPos.x + 10,
                        m_sRoomInUserPosition[i].pCardStartPos.y + 10
                    );
                }

                // 바꾼 카드의 수
                else if ( m_sChgCardInfo[i].l_sUserCardChange.nChangeCardCnt >= 1 && m_sChgCardInfo[i].l_sUserCardChange.nChangeCardCnt <= 4 ) {
                    PutSprite(
                        m_SPR_ChgCardNum,
                        m_sRoomInUserPosition[i].pCardStartPos.x + 10,
                        m_sRoomInUserPosition[i].pCardStartPos.y + 10,
                        m_sChgCardInfo[i].l_sUserCardChange.nChangeCardCnt - 1
                    );
...
void CMainFrame::JokboShow( BYTE *pJokbo, char *pResult )
{
    ...
    switch(pJokbo[0]) {
        ...
        case 3:
            nTemp = ( pJokbo[1] % 13 ) + 1;   // 카드 번호

            if      ( nTemp >= 13 ) { sprintf( szTemp, " 메이드 K탑" ); }
            else if ( nTemp >= 12 ) { sprintf( szTemp, " 메이드 Q탑" ); }
            else if ( nTemp >= 11 ) { sprintf( szTemp, " 메이드 J탑" ); }
            else if ( nTemp <=  1 ) { sprintf( szTemp, " 메이드 A탑" ); }
            else                    { sprintf( szTemp, " 메이드 %d탑", nTemp ); }
            break;
        ...
        case 6:
            ...
            if      ( nTemp >= 13 ) { sprintf( szTemp, " K탑" ); }
            else if ( nTemp >= 12 ) { sprintf( szTemp, " Q탑" ); }
            else if ( nTemp >= 11 ) { sprintf( szTemp, " J탑" ); }
            else if ( nTemp <=  1 ) { sprintf( szTemp, " A탑" ); }
            else                    { sprintf( szTemp, " %d탑", nTemp ); }
            break;
...
void CMainFrame::RenderWaitRoom()
{
	...
	for ( i = m_SCB_WaitRoom->GetStartElem(); i < g_nWaitRoomCnt; i++ ) {
        ...
        if ( bBoldType ) {
            ...
            DrawText( "대박 닐리리~ 즐거운 게임한판!", 250 , nYPos, RGB( 255 , 255 , 255 ), FW_BOLD, TA_LEFT );
        }
        else {
            //DrawText( g_CWaitRoomList.m_sWaitRoomList[nRoomNo].szRoomName, 250 , nYPos, RGB( 255 , 255 , 255 ), FW_NORMAL, TA_LEFT );
            DrawText(  "대박 닐리리~ 즐거운 게임한판!", 250 , nYPos, RGB( 255 , 255 , 255 ), FW_NORMAL, TA_LEFT );
        }
	...
	if ( m_bShowChodaeBox ) {
        DrawInvite( CHODAE_RECV_POS_X, CHODAE_RECV_POS_Y ); // 초대 박스
	}
...
//				break;
		...
		if ( AvailableRoomIn( l_sCreateRoom.biRoomMoney, g_CGameUserList.m_GameUserInfo[0].m_sUserInfo.biUserMoney ) == FALSE ) {
			sprintf( m_szTempText, "유저의 보유톨이 해당방기준과 맞지 않습니다." );
...
*/
    ...
    */
    ...
    if ( m_nUserCallSlotNo == 0 ) {
        // 카드 바꾸기 버튼
		TRACE("카드 바꾸기 버튼 m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt = %d\n",m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt);

        if ( m_ChgCardChgBTN->status == DOWN ) {
            m_ChgCardChgBTN->SetMouseLBDownStatus( FALSE );

            // 바꿀 카드를 선택하지 않고 바꾸기 버튼을 누른 경우
            if ( m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt <= 0 ) {
                SetGameChatText( "◎ 바꿀 카드를 선택해주세요.", RGB( 255, 255, 255 ));
            }
            else {
                m_bMyChgCardTimePassed = TRUE;  // 내 카드 바꾸는 시간이 지나갔음.

                // "바꾸기" 버튼을 누르면 타이머와 버튼을 끈다.
                m_bUserCallTime = FALSE;
                m_nUserCallTime = 0;
                m_nTemp = 0;
			    m_nTimePos = 0;
                ...
                // 바꿀 카드 정보를 서버에 보낸다.
                g_pClient->ChangeUserCard( &m_sChgCardInfo[0].l_sUserCardChange );
                m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt = -1;
        ...
        // 패스 버튼
        if ( m_ChgCardPassBTN->status == DOWN ) {
            // "패스" 버튼을 누르면 타이머를 끈다.
            m_bUserCallTime = FALSE;
            m_nUserCallTime = 0;
            m_nTemp = 0;
...
void CMainFrame::OnLButtonDown()
{
    ...
    if ( m_nCurrentMode == GAME_MODE ) {
		if(m_sTimeMode.nTime ==TTIME_EVENING) {
			...
			if ( g_CGameUserList.m_GameUserInfo[0].m_bDie == FALSE && iMax > -1) {
				if( MouseIn( g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.x,
							 g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y,
							 g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.x + CARD_WIDTH,
							 g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y + CARD_HEIGHT) ) {

					m_ChonHiddenMode = TRUE;

					if(m_ChonCurMousePointYOnHidden <= 0 )
						m_ChonCurMousePointYOnHidden = m_mY;
                    else
						return;
				}
			}
	...
	switch( m_nCurrentMode )
	{
		...
		case GAME_MODE: // 게임화면이면
            ...
            */
            ...
            if ( m_ChgCardChgBTN->status != DISABLED ) {
                m_ChgCardChgBTN->CheckMouseOver( mX, mY, TRUE );
            }

            if ( m_ChgCardPassBTN->status != DISABLED ) {
                m_ChgCardPassBTN->CheckMouseOver( mX, mY, TRUE );
            }
	...
	switch(m_nCurrentMode)
	{
		...
		case GAME_MODE:
            ...
            */
            ...
            if ( m_ChgCardPassBTN->status != DISABLED ) {
                m_ChgCardPassBTN->SetMouseLBDownStatus( FALSE );
            }
...
void CMainFrame::OnMouseMove(UINT nFlags, CPoint point) 
{
    ...
    if (m_nCurrentMode == GAME_MODE ) {
        // 히든 카드 이동 처리/
        if ( /*m_bMoveHiddenCard*/ m_ChonHiddenMode) {
			...
			if(iMax>-1) {
				...
				int TmpCordi    = m_ChonCurMousePointYOnHidden - m_CurHiddenYPos;
				int TmpMoveDist = m_mY-m_ChonCurMousePointYOnHidden;

                TRACE("!!!! m_mY:%d  m_ChonCurMousePointYOnHidden:%d  m_CurHiddenYPos:%d  TmpMoveDist:%d\n",m_mY,m_ChonCurMousePointYOnHidden,m_CurHiddenYPos,TmpMoveDist);
				
				if ( m_CurHiddenYPos  > m_mY) {
				//	TRACE("$$$$$  \n");
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y = m_CurHiddenYPos;
				}
				else if(TmpMoveDist < m_ChonYPosOffset && TmpMoveDist > 0 && !m_ChonHiddenOpenMode){
				//	TRACE("@@@@@  \n");
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y = m_CurHiddenYPos+TmpMoveDist;
				}
				else if(TmpMoveDist >= m_ChonYPosOffset && TmpMoveDist > 0){
				//	TRACE("#####  \n");
				//	g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y   = m_CurHiddenYPos + m_ChonYPosOffset;
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].nCardNo = m_MyHiddenCardNo[iMax];
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y   = m_CurHiddenYPos;
					m_ChonHiddenMode = FALSE;
				}
				/*
				...
				else if(m_mY > m_CurHiddenYPos && m_mY < ( m_CurHiddenYPos + m_ChonYPosOffset )){
					TRACE("@@@@@  \n");
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y = m_mY;
				}
				else if(m_mY > ( m_CurHiddenYPos + m_ChonYPosOffset )){
					TRACE("#####  \n");
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y   = m_CurHiddenYPos + m_ChonYPosOffset;
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].nCardNo = m_MyHiddenCardNo[iMax];
					g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y   = m_CurHiddenYPos;
...
void CMainFrame::CheckButtons(int mX, int mY) // 마우스 커서가 버튼위에 있는지 체크
{
	...
	switch( m_nCurrentMode )
	{
		...
		case GAME_MODE: // 게임화면이면
            ...
            if ( m_ChgCardChgBTN->status != DISABLED ) {
                m_ChgCardChgBTN->CheckMouseOver( mX, mY );
            }

            if ( m_ChgCardPassBTN->status != DISABLED ) {
                m_ChgCardPassBTN->CheckMouseOver( mX, mY );
            }
...
void CMainFrame::SoundLoading()
{
    ...
    g_Sound[GS_MYTURN]          = LoadSndFile(szSndPath, "myturn.wav",        2);
...
BOOL CMainFrame::PreTranslateMessage(MSG* pMsg) 
{
	...
	if( pMsg->message == WM_KEYDOWN ) {
		...
		if ( m_nUserCallSlotNo == 0 ) {
            ...
            if ( m_sTimeMode.nTime >= TTIME_MORNING && m_sTimeMode.nMode == TMODE_CHGCARD )
            {
                if ( m_bMyChgCardTimePassed == FALSE ) {    // 내 차례가 안지나갔으면
                    switch ( pMsg->wParam ) {
                        case VK_NUMPAD1:
                            SelectChangeCard( 0 );
                            break;
...
// 화면 아래쪽에 작은 아침/점심/저녁 아이콘을 그린다.
void CMainFrame::RenderTimeModeSmall()
{
    if ( m_sTimeMode.nTime < TTIME_MORNING || m_sTimeMode.nTime > TTIME_EVENING ) return;

    switch ( m_sTimeMode.nTime ) {
        case TTIME_MORNING: // 아침
            if ( m_sTimeMode.nMode == TMODE_CHGCARD ) {    // 카드 바꾸는 중
                PutSprite( m_SPR_TimeModeSmall, 695, 528, 3 );
            }
            else {
                PutSprite( m_SPR_TimeModeSmall, 695, 528, 0 );
            }
            break;

        case TTIME_AFTERNOON: // 점심
            if ( m_sTimeMode.nMode == TMODE_CHGCARD ) {
                PutSprite( m_SPR_TimeModeSmall, 695, 528, 0 );
                PutSprite( m_SPR_TimeModeSmall, 760, 528, 4 );
            }
            else {
                PutSprite( m_SPR_TimeModeSmall, 695, 528, 0 );
                PutSprite( m_SPR_TimeModeSmall, 760, 528, 1 );
            }
            break;

        case TTIME_EVENING: // 저녁
            if ( m_sTimeMode.nMode == TMODE_CHGCARD ) {
                PutSprite( m_SPR_TimeModeSmall, 695, 528, 0 );
                PutSprite( m_SPR_TimeModeSmall, 760, 528, 1 );
                PutSprite( m_SPR_TimeModeSmall, 825, 528, 5 );
            }
            else {
                PutSprite( m_SPR_TimeModeSmall, 695, 528, 0 );
                PutSprite( m_SPR_TimeModeSmall, 760, 528, 1 );
                PutSprite( m_SPR_TimeModeSmall, 825, 528, 2 );
            }
            break;
    }
}
...
//========================================================================
// 카드 바꾸기 버튼이 작동하지 않게 한다.
//========================================================================

void CMainFrame::AllChgCardBtnDisable()
{
    m_ChgCardChgBTN->status = DISABLED;
    m_ChgCardPassBTN->status = DISABLED;
}
...
void CMainFrame::AllChgCardBtnEnable()
{
    m_ChgCardChgBTN->status = UP;
    m_ChgCardPassBTN->status = UP;
}



//========================================================================
// 마우스 클릭으로 카드를 선택했는지 검사해서 카드 선택 여부를 설정한다.
//========================================================================

void CMainFrame::SetChgCard()
{
    // 혹시 카드가 부족하거나, 카드 바꾸기가 완료되었으면 그냥 종료

    if ( g_CGameUserList.m_GameUserInfo[0].m_nCardCnt < 4 /*||
         m_sChgCardInfo[0].nStatus == 1*/)
    {
        return;
    }

    // 가장 오른쪽 카드부터 왼쪽 카드 방향으로 선택한 카드 검사

    for ( int i = 3; i >= 0; i-- ) {
		/*
		if(g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].nCardNo == 255) {
            g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].nCardNo = ;
		}
		*/
		
        // 잘못된 카드 번호면 처리하지 않는다.
		if(g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].nCardNo != 255) {
			if (( g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].nCardNo < 0 || 
				 g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].nCardNo >= 52))
			{
				continue;
			}
		}

        // 나의 카드 바꾸는 시간이 지나가지 않은 상태에서 카드를 클릭했을 때의 처리.

        if ( MouseIn( g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].Pos.x,
                      g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].Pos.y,
                      g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].Pos.x + CARD_WIDTH,
                      g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[i].Pos.y + CARD_HEIGHT )
             && m_bMyChgCardTimePassed == FALSE )
        {
            SelectChangeCard( i );
            return;
...
//
//------------------------------------------------------------------------
// 이 함수는 서버에서 카드를 바꾸어야할 차례인 유저의 정보를 보내주면
// 실행된다.
// NGSC_CHANGEOKUSER 패킷 참조
//========================================================================

void CMainFrame::SetChgCardUser( int nSlotNo )
{
    // 카드 바꾸기 타이머 시작. 타이머 끝나면 자동 패스 처리
    m_nUserCallTime = 0;
    m_bUserCallTime = TRUE;
    m_nUserCallDefineTime = CHGCARD_TIME;
    m_nTemp = 0;
	m_nTimePos = 0;

    // 카드 바꾸기 모드로 변환
    g_pCMainFrame->m_sTimeMode.nMode = TMODE_CHGCARD;

    // 카드를 바꿀 유저의 번호
    m_nUserCallSlotNo = nSlotNo;

    // 자신의 차례가 되면 바꾸기 버튼을 켜고
    // 자신의 차례가 아니면 바꾸기 버튼을 금지시킨다.
    if ( m_nUserCallSlotNo == 0 ) {
        m_bMyChgCardTimePassed = FALSE; // 내 카드 시간이 안지나갔다고 초기화시킨다.
        AllChgCardBtnEnable();
        PlaySound(g_Sound[GS_MYTURN]);  // 자신의 차례가 되었다고 알린다.
    }
    else {
...
void CMainFrame::SetPassUser( int nSlotNo )
{
    if ( nSlotNo < 0 || nSlotNo >= MAX_ROOM_IN ) {
        return;
    }
	//강 수정처리
	if(nSlotNo == 0 || nSlotNo == 4 || nSlotNo == 3 || nSlotNo == 2 || nSlotNo == 1) {
		m_sChgCardInfo[nSlotNo].l_sUserCardChange.nChangeCardCnt = -1;  // Pass 설정
		m_sChgCardInfo[nSlotNo].nStatus = 1;                            // 카드 바꾸기 완료 설정

	}
    // Pass 처리 완료했다고 서버에 통보한다.
    if ( m_nUserCallSlotNo == 0 ) {
        g_pClient->ChangeUserCardEnd();

     //    m_sChgCardInfo[0].l_sUserCardChange
	     memset( m_sChgCardInfo, 0x00, sizeof(m_sChgCardInfo ));	 //ChonKW 06.03.21			
...
void CMainFrame::ChgUserCard( sUserNewCardPacket *pCardInfo, int nSlotNo )
{
    m_nUserCallSlotNo = nSlotNo;
   
    if ( pCardInfo == NULL || nSlotNo < 0 || nSlotNo >= MAX_ROOM_IN ) {
        return;
    }

    int nChgCardCnt = 0;        // 바꿀 카드의 숫자

    // 카드를 교환하는 유저가 자신이면, 자신이 카드를 바꾼다고 설정해둔다.
    if ( nSlotNo == 0 ) {
        m_bMyChgCard = TRUE;
    }

    // 교환할 새 카드 정보를 유저 정보에 저장
    memset( &g_CGameUserList.m_GameUserInfo[nSlotNo].m_sNewCard, 0x00, sizeof( g_CGameUserList.m_GameUserInfo[nSlotNo].m_sNewCard ));
    memcpy( &g_CGameUserList.m_GameUserInfo[nSlotNo].m_sNewCard, &(pCardInfo->l_sUserNewCard), sizeof( sUserNewCard ));

    for ( int i = 0; i < 4; i++ ) {
        // 바꿀 카드인 경우
        if ( pCardInfo->l_sUserNewCard.nCardSlot[i] != 0 ) {
            // 카드 교체
            //g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].nCardNo = pCardInfo->l_sUserNewCard.nNewCardNo[i];

            // 바꿀 기존 카드를 이동시키기 위한 카드 정보 준비 
            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].ctCardType = CHGCARD;
            
            // 게임 중간에 들어온 사람은 NewPos 값이 카드 덱 위치로 초기화되어 있으므로
            // NewPos 값을 시작위치로 정해서는 안된다.
            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].StartPos.x = m_sRoomInUserPosition[nSlotNo].pCardStartPos.x + ( i * CARD_DISTANCE_A );

            if ( nSlotNo == 0 ) {
                // 자신이면 카드가 약간 올라가 있는 상태에서 움직인다.
                g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].StartPos.y = m_sRoomInUserPosition[nSlotNo].pCardStartPos.y - CHGCARD_UP;
            }
            else {
                // 다른 유저의 카드면 원래 자리부터 카드가 움직인다.
                g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].StartPos.y = m_sRoomInUserPosition[nSlotNo].pCardStartPos.y;
            }

            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].NewPos.x = INIT_CARD_POS_X;
            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].NewPos.y = INIT_CARD_POS_Y;
            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].Pos.x = g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].StartPos.x;
            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].Pos.y = g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].StartPos.y;

            m_CAniMation.AddStack( &g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i] );
            nChgCardCnt++;
        }
    }

    if ( nChgCardCnt > 0 ) {
        m_bChgUpCard = TRUE;  // 바꿀 카드를 에니메이션 스택에 넣고 동기화를 시키기 위해서 설정
		if(nChgCardCnt==0)nChgCardCnt=-1;
        m_sChgCardInfo[nSlotNo].l_sUserCardChange.nChangeCardCnt = nChgCardCnt; // 바꾼 카드 갯수 저장
        m_sChgCardInfo[nSlotNo].nStatus = 1;                                    // 바꾸기 완료 설정

		if(nSlotNo==0)
			TRACE("ChgUserCard nSlotNo %s \n",nSlotNo);
    }
}
...
void CMainFrame::ResetChgCardPos( int nSlotNo ) {
    if ( nSlotNo < 0 || nSlotNo >= MAX_ROOM_IN ) {
        return;
    }
    ...
    // 올려진(선택된) 카드를 아래로 내린다.
    for ( int i = 0; i < 4; i++ ) {
        if ( m_sChgCardInfo[nSlotNo].l_sUserCardChange.nCardSlot[i] != 0 ) {
            g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].Pos.y += CHGCARD_UP;
        }
    }
}
...
void CMainFrame::ClearTimeMode() {
    memset( &m_sTimeMode, 0x00, sizeof( m_sTimeMode ));
}
//========================================================================
// 바꿀패 정보 초기화
//========================================================================
void CMainFrame::SetInitMyChangeCardInfo()
{
       m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt=0;
}

//========================================================================
// 현재 유저의 카드 스택에 새 카드를 넣는다.
//========================================================================

void CMainFrame::AddNewCard()
{

    for ( int i = 0; i < 4; i++ ) {
        if ( g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sNewCard.nCardSlot[i] != 0 ) {

            // 자신이 카드를 받았으면 앞면으로 카드를 넣고,
            // 다른 사람의 카드를 받았으면 뒷면으로 카드를 넣는다.

#ifdef SHOWUSERCARD
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].nCardNo = g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sNewCard.nNewCardNo[i];
#else
            if ( m_nUserCallSlotNo == 0 ) {

				if(m_sTimeMode.nTime ==TTIME_EVENING) {
					m_MyHiddenCardNo[i] = g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sNewCard.nNewCardNo[i];
					g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].nCardNo = 255;
					m_MyHiddenPosition[i] = i;
					m_CurHiddenYPos = 567;
                    m_ChonYPosOffset = 30;
            ...
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].ctCardType = FLOOR;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].nAniCount = 0;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].nAniMaxCount = -1;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].StartPos.x = INIT_CARD_POS_X;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].StartPos.y = INIT_CARD_POS_Y;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].Pos.x = INIT_CARD_POS_X;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].Pos.y = INIT_CARD_POS_Y;
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].NewPos.x = m_sRoomInUserPosition[m_nUserCallSlotNo].pCardStartPos.x + ( i * CARD_DISTANCE_A );
            g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i].NewPos.y = m_sRoomInUserPosition[m_nUserCallSlotNo].pCardStartPos.y;
            m_CAniMation.AddStack( &g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[i] );
        }
    }
    ...
    if(m_nUserCallSlotNo == 0 ) {
		for(u=0;u<4;u++) {
			cCardNo = g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[u].nCardNo;

			if(cCardNo == 255 && cReadNo < iCount ) {
				g_CGameUserList.m_GameUserInfo[m_nUserCallSlotNo].m_sUserCardInfo[u].nCardNo = m_MyHiddenCardNo[u];
				cReadNo++;
			}
	...
	if(iMax>0 && iCount>0) {
			m_SPR_Arrow->SetPos( g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.x ,
								 g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[iMax].Pos.y - 12);
			m_SPR_Arrow->Draw( g_pCDXScreen->GetBack(), 0, 0, CDXBLT_TRANS );	
	}

    // 새로 교환하는 카드가 자신의 카드이면(m_bMyChgCard == TRUE)
    // m_bChgDownCard 를 TRUE로 설정한다. m_bChgDownCard 는 카드가
    // 유저 자신에게 도착했는지를 알기위해서 사용한다. 바꾸려는 카드가
    // 내 카드이고(m_bMyChgCard == TRUE), 카드가
    // 도착했으면(m_bChgDownCard == TRUE &&  m_CAniMation.GetAniCount() < 0)
    // 서버에게 카드 받는 것이 끝났다고 알려준다(g_pClient->ChangeUserCardEnd()).
    
    if ( m_bMyChgCard == TRUE ) {
        m_bChgDownCard = TRUE;
    }
}
...
void CMainFrame::SetGameCurUser()
{
    int nSlotNo = m_sNextUserInfo.nSlotNo;

    g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserInfo.biUserMoney = m_sNextUserInfo.l_sNextUser.biBeforeGameUserMoney;

	UserSelectView( nSlotNo, m_sNextUserInfo.l_sNextUser.cBeforeUserSelect, TRUE );
	
#ifndef SHOWUSERCARD
	//이전 유저가 Die를 하였으면 Die한 유저의 카드를 덮는다.
	if ( m_sNextUserInfo.l_sNextUser.cBeforeUserSelect == '7' ) {
	...
	g_pCMainFrame->SetPanMoney( nSlotNo, m_sNextUserInfo.l_sNextUser.biRoomCurMoney);
	
	nSlotNo = g_CGameUserList.FindUser( m_sNextUserInfo.l_sNextUser.nCurGameUserUniNo );
	g_pCMainFrame->SetUserCall( m_sNextUserInfo.l_sNextUser.l_sGameCurUser, nSlotNo );
}
...
// "패스" 버튼 눌렸을 때의 처리
//------------------------------------------------------------------------
// 타이머를 끈다던가, 카드 바꾸기 버튼을 Disable 시킨다던가하는
// 부수적인 작업은 수행하지 않고 "패스" 처리에 관련된 작업만 한다.
//========================================================================

void CMainFrame::ProcessPassBTN()
{
    m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt = -1;
    ResetChgCardPos( 0 );       // 선택되었던 카드 위치를 제자리로 이동
    g_pClient->ChangeUserCard( &m_sChgCardInfo[0].l_sUserCardChange );
...
//------------------------------------------------------------------------
// input:
//      nIdx = 선택할 카드 인덱스 번호(4장이니까 0~3)
//------------------------------------------------------------------------
// SelectChangeCard() 함수에서 사용된다.
//========================================================================

void CMainFrame::SelOneChgCard( int nIdx )
{
    if ( nIdx >= 0 && nIdx < 4 ) {
		if(m_sTimeMode.nTime ==TTIME_EVENING) {
			int iMax=-1;
			...
			if(g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[nIdx].nCardNo == 255 && iMax != nIdx && iMax != -1) {
				g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[nIdx].nCardNo = m_MyHiddenCardNo[nIdx];
				return;
			}
		}
		
        m_sChgCardInfo[0].l_sUserCardChange.nCardNo[nIdx] = g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[nIdx].nCardNo;
        m_sChgCardInfo[0].l_sUserCardChange.nCardSlot[nIdx] = 1;
	//	TRACE("SelOneChgCard Select Count  %d \n",m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt);
        m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt++;
		TRACE("SelOneChgCard Select Count  %d \n",m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt);
		...
		g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[nIdx].Pos.y = m_sRoomInUserPosition[0].pCardStartPos.y - CHGCARD_UP;   // 카드를 위로 조금 올림
        
		for(int u=0;u<4;u++) {
			
			iMax2 = m_MyHiddenPosition[u];
			if(iMax2>-1) {
				g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[nIdx].Pos.y = m_CurHiddenYPos;
			}
			iMax = (m_MyHiddenPosition[u]>iMax) ? m_MyHiddenPosition[u] : iMax;
		...
		if(iCnt == iMax && iMax2>-1)
			m_ChonHiddenOpenMode = TRUE;
	}
}
...
void CMainFrame::DeselOneChgCard( int nIdx )
{
    if ( nIdx >= 0 && nIdx < 4 ) {
        m_sChgCardInfo[0].l_sUserCardChange.nCardSlot[nIdx] = 0;
        m_sChgCardInfo[0].l_sUserCardChange.nCardNo[nIdx] = 0;
        m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt--;
        g_CGameUserList.m_GameUserInfo[0].m_sUserCardInfo[nIdx].Pos.y = m_sRoomInUserPosition[0].pCardStartPos.y;  // 카드를 원래 자리로 이동
    }
}
...
//========================================================================
// 지정된 카드를 선택하거나, 선택을 해제한다.
//------------------------------------------------------------------------
// input:
//      nIdx = 선택할 카드 인덱스 번호(4장이니까 0~3)
//========================================================================

void CMainFrame::SelectChangeCard( int nIdx )
{
    // 선택되어 있지 않은 카드를 선택한 경우
    if ( m_sChgCardInfo[0].l_sUserCardChange.nCardSlot[nIdx] == 0 ) {
        SelOneChgCard( nIdx );
        /*
        // 아침에는 3장, 점심에는 2장, 저녁에는 1장만 바꿀 수 있다.
        if ( m_sTimeMode.nTime == TTIME_MORNING && m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt >= CHGCNT_MORNING ||
             m_sTimeMode.nTime == TTIME_AFTERNOON && m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt >= CHGCNT_AFTERNOON ||
             m_sTimeMode.nTime == TTIME_EVENING && m_sChgCardInfo[0].l_sUserCardChange.nChangeCardCnt >= CHGCNT_EVENING )
        {
            return;
        }
        else {
...
int CMainFrame::GetUserCardCnt( int nUserIdx )
{
    int nCardCnt = 0;

    for ( int i = 0; i < g_CGameUserList.m_GameUserInfo[nUserIdx].m_nCardCnt; i++ ) {
        if ( g_CGameUserList.m_GameUserInfo[nUserIdx].m_sUserCardInfo[i].ctCardType == USERCARD ) {
            nCardCnt++;
        }
    }
...
//========================================================================
// 임시로 저장되어 있던 내 족보를 업데이트한다.
//========================================================================

void CMainFrame::UpdateMyJokBo()
{
    memcpy( g_CGameUserList.m_GameUserInfo[0].m_szJokboCard, &m_sTempMyUserJokBo, sizeof( m_sTempMyUserJokBo ));
    g_CGameUserList.m_GameUserInfo[0].m_bEnd = true;
    m_bChgMyJokBo = FALSE;
}
...
void CMainFrame::ChangeUserOneCard( int nSlotNo, sUserChange *pChgCardInfo )
{
    ...
    for ( int i = 0; i < g_CGameUserList.m_GameUserInfo[nSlotNo].m_nCardCnt; i++ ) {
        if ( g_CGameUserList.m_GameUserInfo[nSlotNo].m_sUserCardInfo[i].nCardNo == pChgCardInfo->bUerCardNo ) {
            ...
            // 바꿀 카드가 카드를 교환하기 위해 선택된 카드이면 관련 정보를 넣어준다.
            if ( m_sChgCardInfo[nSlotNo].l_sUserCardChange.nChangeCardCnt >= 1 &&
                 m_sChgCardInfo[nSlotNo].l_sUserCardChange.nCardSlot[i] == 1 )
            {
                m_sChgCardInfo[0].l_sUserCardChange.nCardNo[i] = pChgCardInfo->bChangeCardNo;
            }

...
bool ImageEx::TestForAnimatedGIF()
{
   ...
   */
	m_nFrameCount = GetFrameCount(&FrameDimensionTime);	        // 아바타의 프레임 수 구해서 저장
	int nSize = GetPropertyItemSize(PropertyTagFrameDelay);
	m_pPropertyItem = (PropertyItem*) malloc(nSize);
...
void ImageEx::Initialize()
{
	...
	m_bPause     = false;
	m_hExitEvent = CreateEvent(NULL, TRUE, FALSE, NULL);    // 수동 리셋 이벤트(생성 후 바로 신호 대기 아님)
	m_hPause     = CreateEvent(NULL, TRUE, TRUE,  NULL);    // 수동 리셋 이벤트(생성 후 바로 신호 대기)
}
...
{
	if ( m_SPR_Avatar == NULL ) {
		return false;
	}

	::WaitForSingleObject(m_hPause, INFINITE);

	GUID pageGuid = FrameDimensionTime; // FrameDimensionTime <-- Predefined multi-frame dimension IDs

	long hmWidth = GetWidth();          // 이미지의 가로 크기를 구한다.
	long hmHeight = GetHeight();        // 이미지의 세로 크기를 구한다.
	HDC hDC = NULL;
    ...
    hDC = m_SPR_Avatar->GetTile()->GetDC();

    // m_SPR_Avatar의 표면이 파괴되면 아바타 에니메이션 스레드를 멈춘다.
    ...
    // m_SPR_Avatar에 아바타의 현재 프레임을 그린다.

    else {
		Graphics graph( hDC );
		graph.Clear(Color( 161, 207, 99 ));                         // 아바타 이미지의 배경색을 바꾼다.
        graph.DrawImage( this, m_pt.x, m_pt.y, hmWidth, hmHeight ); // 아바타를 그려넣는다.
		m_SPR_Avatar->GetTile()->ReleaseDC();
	}
	
	SelectActiveFrame(&pageGuid, m_nFramePosition++);   // 이미지 프레임 선택
	
	if (m_nFramePosition == m_nFrameCount)              // 마지막 프레임이면 첫 번째 프레임으로 이동
		m_nFramePosition = 0;

...
public:
	...
	int m_nTemp;
    int m_nGoodsZoom;               // "경품 당첨" 이미지 확대 여부 설정(0 = 보통 크기, 1 = 확대 크기)
    int m_nGoodsZoomCnt;            // "경품 당첨" 이미지의 확대 여부를 바꾸기 위해 시간을 계산하는데 사용
    int m_nGoodsZoomShow;           //
    int m_nDrawWinner[MAX_ROOM_IN]; // 비긴 유저들 설정(1이면 비긴 유저, 0이면 지거나 없는 유저)
    ...
    BOOL m_bChgUpCard;          // 새로운 카드로 바꿀 기존 카드가 목적지에 도착했는지 표시
    BOOL m_bChgDownCard;        // 새로운 카드가 유저에게 도착했는지 표시
    BOOL m_bMyChgCard;          // 교환된 카드가 내 카드인지의 여부를 설정한다.
    BOOL m_bMyChgCardTimePassed;    // 내 카드 교환 시간이 지나갔는지의 여부를 설정.
                                    // 마우스 클릭과, 타이머간의 동기화를 위해 사용
    BOOL m_bChgMyJokBo;         // 서버로 부터 받아서 저장하고 있던 족보로 갱신할지 말지를 나타냄
    BOOL m_bDrawGame;           // 비긴 게임인지 표시(NGSC_ENDGAME에서 설정됨)

    BOOL m_IsMyTurn;

	BYTE m_MyHiddenCardNo[4];
	int m_MyHiddenPosition[4];
	int m_CurHiddenYPos;
	int m_ChonYPosOffset;
	int m_ChonCurMousePointYOnHidden;
    BOOL m_ChonHiddenMode;
    BOOL m_ChonHiddenOpenMode;
    ...
    sTimeMode m_sTimeMode;                              // 아침, 점심, 저녁 및 베팅중, 카드바꾸는 중 여부 설정
    sChgCardInfo m_sChgCardInfo[MAX_ROOM_IN];           // 바꿀 카드의 정보 저장용 구조체
    sNextUserInfo m_sNextUserInfo;                      // 다음 유저를 현재 유저로 설정하기 위해 사용(동기화 위해)
    sMyUserJokBo m_sTempMyUserJokBo;                    // 유저 자신의 족보(패킷이 전달될 때마다 여기에 임시로 저장됨)
    sStartUserSync m_sStartUserSync;                    // 게임을 처음 진행하는 유저의 시작 타이밍을 위해 사용(동기화)
...
//Chonkw 추가 
    ...
    CDXSprite *m_SPR_WindowDraw;            // 비긴 결과 화면
	...
	CDXSprite *m_SPR_TimeModeBig;				// 아침, 점심, 저녁 큰 이미지
	CDXSprite *m_SPR_TimeModeSmall;				// 아침, 점심, 저녁 작은 이미지
    CDXSprite *m_SPR_ChgCardNum;                // 바꾼 카드 숫자 출력용 숫자 이미지
    CDXSprite *m_SPR_ChgCardPass;               // 바꾼 카드 없을 때 출력하는 Pass 이미지
    CDXSprite *m_SPR_SelCardKey;                // 바꿀 카드 선택하는 단축키
    CDXSprite *m_SPR_BetBtnKey;                 // 베팅 버튼 단축 키 글자
    CDXSprite *m_SPR_ChgCardMsg;                // 바꾸기 버튼 아래에 출력되는 메시지
    ...
    CDXButton *m_ChgCardChgBTN;                 // 카드 바꾸기 "바꾸기 버튼"
    CDXButton *m_ChgCardPassBTN;                // 카드 바꾸기 "패스 버튼"
...
/////////////////////////////////////////////
    ...
    void RenderDrawWinner();                // 비긴 우승자들을 출력한다.
	void RenderTimePos();
	void RenderUserCard();
    ...
    void RenderTimeModeSmall();                     // 아침, 점심, 저녁 작은 아이콘을 그린다.
    ...
    void AllChgCardBtnDisable();            // 카드 바꾸기 버튼 사용 금지시키기
    void AllChgCardBtnEnable();             // 카드 바꾸기 버튼 사용 가능하게한다.
    void SetChgCard();                      // 선택한 카드 설정
    void SetChgCardUser( int nSlotNo );     // 카드 바꿀 유저 선택
    void SetPassUser( int nSlotNo );        // 패스한 유저 설정
    void ChgUserCard( sUserNewCardPacket *pCardInfo, int nSlotNo );  // 유저 카드를 바꾼다.
    void ResetChgCardPos( int nSlotNo );    // 선택되었던 카드의 위치를 원래대로 이동시킨다.
    void ClearTimeMode();                   // 아침, 점심, 저녁 관련 모드를 초기화한다.
    void AddNewCard();                      // 현재 유저의 카드 스택에 새 카드를 넣는다.
    void SetGameCurUser();                  // 현재 유저 설정
    void ProcessPassBTN();                  // 패스 버튼 처리
    void SelOneChgCard( int nIdx );         // 바꿀 카드 선택
    void DeselOneChgCard( int nIdx );       // 바꿀 카드 선택 해제
    void SelectChangeCard( int nIdx );      // 지정된 위치의 카드를 선택하거나, 선택해제한다.
    int GetUserCardCnt( int nUserIdx );     // 도착해 있는 카드가 몇 장인지 알아본다.
    void UpdateMyJokBo();                   // 자신의 족보를 임시로 저장되어 있던 내용으로 업데이트한다.

#ifdef CHANGEUSERCARD
    BOOL CheckChatCommand(char *szText);                                // 채팅 명령어 처리 함수
    void ChangeUserOneCard( int nSlotNo, sUserChange *pChgCardInfo );   // 카드 바꾸는 함수
